...
public class Acti_Xiaomi extends AppCompatActivity {
    ...
    private EditText edtTimKiemXiao;

    private TextView btnip, btnss, btnvv, btnxm, btnop;
    private TextView noResultsTextView;

    private static final String HANG_SX_ID = "6675aa1eff75b8dfd1e641ea"; // Oppo's manufacturer ID
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        noResultsTextView = findViewById(R.id.tv_no_resultsXiao); // Initialize TextView for no results
        ...
        edtTimKiemXiao = findViewById(R.id.edtTimKiemXiaomi);
        ...
        edtTimKiemXiao.addTextChangedListener(new TextWatcher() {
            @Override
        ...
        edtTimKiemXiao.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    if (event.getRawX() >= (edtTimKiemXiao.getRight() - edtTimKiemXiao.getCompoundDrawables()[2].getBounds().width())) {
                        // Xóa nội dung của EditText
                        edtTimKiemXiao.setText("");
            ...
            public void onResponse(Call<List<ProductHome>> call, Response<List<ProductHome>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (!filteredProducts.isEmpty()) {
                        adapter = new IphoneAdapter(Acti_Xiaomi.this, filteredProducts, noResultsTextView);
                        recyclerView.setAdapter(adapter);
                    } else {
